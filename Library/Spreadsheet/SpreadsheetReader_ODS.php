 ...
 */
	class SpreadsheetReader_ODS implements Iterator, Countable
	{
		...
		public $Pictures = array(); //массив картинок
		...
		public function __construct($Filepath, array $Options = null)
		{
			...
			{
				$Zip -> extractTo($this -> TempDir); //разархивировать все
				//$Zip -> extractTo($this -> TempDir, 'content.xml');
				$this -> ContentPath = $this -> TempDir.'content.xml';
				//доставть все картинки
				//$Zip -> extractTo($this -> TempDir, 'Pictures');
		...
		public function __destruct()
		{
			...
			{
				//@unlink($this -> TempDir);	//удалить всю папку с картинками
					...
					switch ($this -> Content -> name)
					{
						...
						case 'draw:image':
							...
							if ($this -> Content -> nodeType != XMLReader::END_ELEMENT)
							{
								...
								$LastCellContent = "img=".$this -> PicturePath .$this -> Content -> getAttribute('xlink:href');
								$CellValue = $LastCellContent;
								//$LastCellContent="img";
								//добавить в массив картинок
								$this -> Pictures[] = $this -> PicturePath . $this -> Content -> getAttribute('xlink:href');
								//echo var_dump($this -> Pictures);
								//echo $this -> Content -> getAttribute('xlink:href');
								//echo $LastCellContent;
							}
							break;
						case 'table:table-row':
							$this -> RowOpen = false;
