 ...
 */
	class SpreadsheetReader_XLSX implements Iterator, Countable
	{
		...
		private $PicturePath = '';
		private $Pictures = array(); //массив картинок по строкам из drawing1.xml
		private $PicturesIds = array("123"=>"123");//идентификаторы картинок из drawing1.xml.rels

		public function GetPictures(){
			return $this -> Pictures;
		}
		//$i = row
		public function GetPicture($i){
...
//for ($i=0; $i<$Zip->numFiles;$i++) {        var_dump($Zip->statIndex($i));}
			...
			if ($Zip -> locateName('xl/styles.xml') !== false)
			{
				...
				// Media - id of images
				if ($Zip -> locateName('xl/drawings/_rels/drawing1.xml.rels') !== false)
				{
					$this -> MediaRels= new SimpleXMLElement($Zip -> getFromName('xl/drawings/_rels/drawing1.xml.rels'));
					$this -> PicturePath = $this -> TempDir;

					if ($this -> MediaRels -> Relationship)
					{
						foreach ($this -> MediaRels -> Relationship as $Index => $r)
						{
							$id = (string)$r->attributes()->Id;
							$img = (string)$r->attributes()->Target;
							$ext = strtolower(substr($img, strrpos($img, '.') + 1));
							if($ext == "png" or $ext == "jpg" or $ext == "jpeg"){

								//save img
								$tempFile = $img;
								$tempFile = str_replace('media','',$tempFile);
								$tempFile = str_replace('..','',$tempFile);
								$tempFile = str_replace('//','/',$tempFile);
								$targetFile =  str_replace('//','/',$this -> TempDir) . $tempFile;
								$targetFile = str_replace('//','/',$targetFile);
								//var_dump($tempFile);
								//var_dump($targetFile);
								if($Zip -> locateName('xl/media'.$tempFile) !== false){

									$im_string = $Zip->getFromName('xl/media'.$tempFile);
									$im = imagecreatefromstring($im_string);
									imagepng($im, $targetFile);

									$this->PicturesIds[$id] = $targetFile;
									//var_dump($targetFile);
									
								}

							}else{
								var_dump("Wrong image extention: ".$ext);
							}
						}
					}
				}
				// media - rows of images by id
				if ($Zip -> locateName('xl/drawings/drawing1.xml') !== false)
				{
					//xml-схема путанная и может меняться, поэтому парсим так
					$this -> Medias= new SimpleXMLElement($Zip -> getFromName('xl/drawings/drawing1.xml'));
					$source = $this -> Medias -> asXML();
					$img_xmls = explode('<xdr:twoCellAnchor',$source);
					foreach ( $img_xmls as $img_xml){
						//echo "\n\n\n\ ------------------- \n";
						
						//var_dump($img_xml);
						//$row = 0;
						//$col = 0;
						//$img_id = ""; // id картинки из $this->PicturesIds

						preg_match("/<xdr:row>(\d+)<\/xdr:row>/",$img_xml, $row);
						//var_dump($row);
						$row = $row[1];

						//не используется
						preg_match("/<xdr:col>(\d+)<\/xdr:col>/",$img_xml, $col);
						//var_dump($col);
						$col = $col[1];

						preg_match("/r:embed=\"([a-zA-Z0-9]+)?\"/", $img_xml, $img_id);
						//var_dump($img_id);
						$img_id = $img_id[1];
						...
						if($row && $img_id){
							//var_dump($row);
							//var_dump($img_id);
//var_dump($this->PicturesIds[$img_id]);
							$this->Pictures[$row] = $this->PicturesIds[$img_id];
				...
				while ($this -> Valid = $this -> Worksheet -> read())
				{
					switch ($this -> Worksheet -> name)
					{
						...
						case 'is':
							...
							break;
						//default:
							//var_dump($this -> Worksheet -> name);
						//	var_dump($this -> Worksheet -> readString);
					}
				}
