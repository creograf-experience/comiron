...
class Image {
    ...
    if (isset($factor) && $factor < 1) {
      ...
      if (function_exists('symlink')) {
        if (! symlink($file_path, $thumb)) {
          //die("Permission denied on generating thumbnail symlink ($file_path, $thumb)");
          return false;
        }
      } else {
        // php on windows doesn't know how to symlink so copy instead
        if (! copy($file_path, $thumb)) {
          //die("Permission denied on generating thumbnail copy ($file_path,$thumb)");
        	return false;
        }
      }
    }
  ...
  static public function createImage($source, $destination, $desired_width, $desired_height) {
    ...
    if ($desired_width && $desired_height) {
	    ...
	    //если изображение меньше, чем thumbnail, то оставить оригинальный размер
	    /*if($desired_width>$info[0] and $desired_height>$info[1]){
	    	$desired_width=$info[0]-1;
	    	$desired_height=$info[1]-1;
	    }*/
