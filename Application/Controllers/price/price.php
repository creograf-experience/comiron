...
class priceController extends baseController {
        ...
        if($method_POST['state']){
            ...
            // $for_developer['log'] = $for_developer['log'] . $update_tokens['message'];
                ...
                //    $for_developer['log'] = $for_developer['log'] . $vaildate_message;
                    ...
                    $this->send_push($price_id, $data, $result);
                    $price_model->update_price_cash($data['id_shop'],$price_id);
    ...
    private function send_push($price_id, $data, $result){
      if(!$price_id or !($data['id_clients'] or $data['send2all'])){ return; }

        $price_push = $this->model("price_push");
        $person_id_send = $price_push->get_persons($price_id);  //person_id которым уже отправляли пуши
        $push_send=[];
        ...
        $notclients = [];
        $clients = $shop_model->get_clients($data['id_shop']);
        if($data['send2all']){
          $data['id_clients'] = array_keys($clients);
        }


        //убрать тех, кому уже отправляли пуши
        $data['id_clients'] = array_diff($data['id_clients'], $person_id_send);
//var_dump($data['id_clients']);
        foreach($data['id_clients'] as $person_id){
          // не подписан
          if(!in_array($person_id, array_keys($clients))){
          ...
          $notification["to"]=$key; //json_encode($t
...
//var_dump($push_send);
        if(count($push_send)>0){
          $price_push->add_persons($data['id_shop'],$price_id,$push_send);
        }
...
//var_dump($data);
                if($data['shop_id']){
                  $data['id_shop']=$data['shop_id'];
                }
                $update_price = $price_model->update_price($data);
                $this->send_push($data['id'], $data, $update_price);
                $price_model->update_price_cash($data['id_shop'], $data['id']);
        ...
        if($method_GET['state']){
            ...
            // if ($validate_tokens['state']){
                ...
                if (array_key_exists("page",$_REQUEST)){
                  $page = $_REQUEST['page'];
                  $data = $price_model->get_price_detail_page($shop_id, $price_id,$user_id,$group_id,$property, $page);
                }else{
	      ...
	      if($data["person_id"]){
              ...
              //if($valid_request_json['state']){
                  ...
                  if($order_id){
                    ...
                    $this->sendXML($order_id,$shop_id,$sum,$person_id,$cart,$data);
...
//var_dump($data);
      ...
      if(isset($id) and is_numeric($id)){
        ...
        if($data['products']){
          ...
          if($data["person_id"]){
	      $this->sendXML($id,$data['shop_id'],$data['sum'],$data['person_id'],$cart,$data);
	  }
        }
    ...
    public function updatetobuy($params){
        ...
        if($method_POST['state']){
                ...
                $price_model = $this->model('price');
                $result = $price_model->updatetobuy($price_id, $data);
    ...
    public function sz_generate_xls($params){
      ...
      $people = $this->model('people');
      $shops = $this->model('shop');
      $priceorder = $this->model('order'); //price
    ...
    public function update_price_cash($params){
      $shop_id = $params[3];
      $price_id = $params[4];
      $price_model = $this->model('price');

      if($shop_id && $price_id){
        $price_model->update_price_cash($shop_id,$price_id);
      }else{
        global $db;
    ...
    private function sendXML($order_id,$shop_id,$sum,$person_id,$cart,$data){
      $priceorder = $this->model("orderprice");
  ...
  /*        $sheet->getStyleByColumnAndRow($i - 2, $j)->getAlignment()->
      ...
      $message=$this->get_template("/shop/mail/neworderprice.php",
          array("cart"=>$cart,
            ...
            "delivery"=>"",
            "phone"=>$data['phone'],
            "comment_person"=>$data['comment_person'],
              ));
      global $mail;
      $mail->send_mail(array(
          "from"=>PartuzaConfig::get('mail_from'),
