    ...
    if (count($_REQUEST)) {
      ...
      */
      ...
      if(isset($res[1]) and isset($res[2]) and isset($res[3])){
      	$_REQUEST['date_of_birth']=mktime(0, 0, 1, $res[2], $res[1], $res[3]);
      }

      try {
        $people->save_person($_SESSION['id'], $_REQUEST);
...
public function get_id_by_email($params) {
	$people = $this->model('people');
	$id = $people->get_id_by_email($params[3]);
	echo "{\"id\":".(($id)?$id:0)."}";
}

public function partizan($params) {
	$data = json_decode(file_get_contents('php://input'), true);

	if(!$data['phone'] or !$data['partizan_uid']){
		echo json_encode(["success"=>true, "id"=>null]);
		return;
	}

	$phone = $data['phone'];
	$partizan_uid = $data['partizan_uid'];

	$people = $this->model('people');
	$id = $people->get_id_by_phone($phone);
	$result = ["success"=>true, "id"=>null];

	if($id){
		$people->save_person(id, ["partizan_uid"=>$partizan_uid]);
		$result["id"]=$id;
	}
  ...
  public function verifyphone() {
	...
	try {
	  ...
	  $id = $people->get_id_by_phone($phone);
      if (!$id) {
		echo json_encode(['error' => 'Persons not founded']);
        return;
	  }
	
	  $res = $people->activate_param('phone', $phone);
	  echo json_encode(['is_activated' => $res]); 
