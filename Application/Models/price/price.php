...
class priceModel extends Model {
    private $table = "price";
    private $supported_fields = array('id', 'id_shop', 'date', 'name','descr',"tags_id", "enddate", "is_sz","send2all","status", "groups", "productproperties");
    private $perpage = 50;
...
//        $query_chunk = " WHERE p.id_shop=%d and p.id=%d and c.id_price=%d and b.id_price=%d ";
    ...
    private function fetch_select_price_detail($res, $db){
        ...
        while ($query_item = $db->fetch_array($res, MYSQLI_ASSOC)) {
            ...
            $price_detail['groups'] = json_decode($query_item['groups']);
            $price_detail['productproperties'] = json_decode($query_item['productproperties']);
...
//            $response_array['id'] = $db->mysql_insert_id();
    ...
    private function get_product_instances($db, $shop_id, $id_products){
        ...
        $query_mold = "SELECT `id`, tobuy, `price` as cost, `edizm`, `sklad` as count, `currency_id` as currency_id FROM `product` WHERE shop_id=$shop_id ".$sql;
        $i=0;
    ...
    public function update_price($updated_data){
      ...
      if($updated_data['shop_id']){
        $updated_data['id_shop']=$updated_data['shop_id'];
      }
      $shop_id = $db->addslashes($updated_data['id_shop']);
...
//var_dump($query);
      ...
      if(array_key_exists("id_clients",$updated_data)){
        ...
        $sql = "select client_id from shop_clients
                where shop_id=$shop_id and client_id in (".implode(',',$updated_data['id_clients']).")";
...
//      	}
        ...
        $query = "select distinct id from ".$this->table." where "
        .((count($price_ids)>0) ?" id in (".implode(',', $price_ids).") ":" 0 ")
...
//var_dump($result['id_products']);
        ...
        $group = $this->get_model("group");
        $cart_sz = $this->get_model("cart_sz");
...
//var_dump($query);
          ...
          $data['num_orders_sz'] = $cart_sz->get_orders_sz_count($data['id']);

          //количество заказов на пользователя
          $data["num_orders"]="0";
          $query = "select sum(orderdetail.num) as res from orderdetail, `order`
             where orderdetail.order_id = order.id and order.person_id=".$user_id."
              and orderdetail.product_id=".$data['id'];
//var_dump($query);
          $r = $db->query($query);
          if ($r->num_rows) {
            $r = $r->fetch_array(MYSQLI_ASSOC);
         ...
         // $data[num_orders]=0;
          $data["product"] = $productM->get_product($data['id'], false);
/*
          //свойства
...
//var_dump($allgroups_ids);
    ...
    public function get_price_detail_page($shop_id, $id_price, $user_id,$group_id = 0, $properties = 0, $page = 0) {
        global $db;

        $response_array['state'] = false;
        $response_array['message'] = __FUNCTION__." return false... ";

	      $query = "SELECT p.id_shop,p.id, p.date, p.name, p.descr,p.tags_id, p.enddate, p.is_sz, p.send2all,
                p.groups, p.productproperties,
                GROUP_CONCAT(distinct b.id_product ORDER BY b.id_product asc SEPARATOR ',') as id_products
                FROM `price` p
                LEFT JOIN `price_products` b ON p.id=b.id_price
                where p.id_shop=$shop_id and p.id=$id_price
                GROUP BY p.id, p.id_shop
                LIMIT 1
                ";
        ...
        $result['productproperties'] = json_decode($result['productproperties']);
        $result['tags_id'] = json_decode($result['tags_id']);
        $result['id_products']= array_map('intval', explode(',', $result['id_products']));                            
	...
	//пагинация
	//$ar = explode(",",$result['id_products']);
        $result['id_products'] = array_slice($result['id_products'], $this->perpage * $page, $this->perpage);
        //$result['id_products'] = implode(",",$ar);
...
//var_dump($allgroups_ids);
    ...
    public function delete_by_price_id($id){
    	...
    	$id = $db->addslashes($id);
      $res = $db->query("delete from id_clients where id_price=$id");
      $res = $db->query("delete from price_push where price_id=$id");
    	$res = $db->query("delete from price_products where id_price=$id");
...
#var_dump($query);
    ...
    public function addproducts($updated_data){
    	global $db;
      $id = $db->addslashes($updated_data['id']);
      ...
      $updated_data['id_shop']=$updated_data['shop_id'];
        ...
        //$res = $db->query("delete from price_products where id_price=".$id);
        //alter table `price_products` add CONSTRAINT UNIQUE KEY (id_price, id_product);
        $products_instanse = $this->get_product_instances($db, $shop_id, $updated_data['id_products']);
    ...
    public function update_price_cash($shop_id, $price_id) {
        global $db;
	$db->query("SET GLOBAL group_concat_max_len = 1000000");

        $query = $this->make_select_price_detail($shop_id, $price_id);
        $res = $db->query($query);
//var_dump($query);
        $result = $this->fetch_select_price_detail($res, $db);
        ...
        foreach($result[0]['id_products'] as $product_id){
          // свойства
          $props=$property->get_props($product_id);
          foreach($props as $prop){
             $allprops_ids[]=$prop['id']; //id родительской группы товара
          }
          // группы
          $groups=$group->get_groups_of_product($product_id);
          if($groups[0]['id']){
      	     $allgroups_ids[]=$groups[0]['id']; //id родительской группы товара
      	  }

        }

        $groups = $db->addslashes(json_encode(count($allgroups_ids)>0 ? $group->get_tree_for_groups($shop_id, $allgroups_ids) : [] ));
        $productproperties = $db->addslashes(json_encode(count($allprops_ids)>0 ?$property->get_properties_distinct($allprops_ids) : []));
        //сохранить json в базу
        $query = "update ".$this->table." set groups=\"$groups\", productproperties=\"$productproperties\" where id=$price_id";
        $res = $db->query($query);
    }
