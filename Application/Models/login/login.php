...
class loginModel extends Model {
  ...
  public function authenticate($email, $password) {
    ...
    $res = $db->query("select id, first_name, last_name, email, thumbnail_url, thumbnail_url_big  from persons where email = '$email' and password = PASSWORD('$password')");
    if ($db->num_rows($res)) {
  ...
  public function get_user($id) {
    ...
    $email = $db->addslashes($email);
    $password = $db->addslashes($password);
    $res = $db->query("select id, first_name, last_name, email, thumbnail_url, thumbnail_url_big  from persons where id=$id");
    if ($db->num_rows($res)) {
      return $db->fetch_array($res, MYSQLI_ASSOC);
    } else {
      return false;
