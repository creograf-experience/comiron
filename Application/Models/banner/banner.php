...
class bannerModel extends Model {
	private $table="`shop_banners`";
  	//public $cachable = array('get_banners');
  	public $supported_fields = array('id', 'link', 'shop_id','isvisible','ordr','shown', 'clicked', 'img');

  
  //подготовить список 
  public function load_get_banners($shop_id=0){
  	global $db;

	//показаны
	$query="update ".$this->table." set shown = shown + 1 where shop_id=$shop_id";
  	$res = $db->query($query);
  	
	$query="select * from ".$this->table." where shop_id=$shop_id and isvisible>=0 order by ordr";
  	$res = $db->query($query);
