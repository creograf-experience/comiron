...
foreach ($vars['news'] as $news) {
          ...
          if(!empty($news['shop_id'])) {
		    ...
		    echo "<div class=\"new_title_line\">";
 		    $qwtitle=stripslashes($news['title']);
   			if(!empty($news['shop_id'])) {
            	echo "<a href=\"/shop/{$news['shop_id']}\">{$news['name']}</a> | <a class=\"newsopen\" title=\"{$qwtitle}\" href=\"/shop/news/get/{$news['id']}\">{$news['title']}</a>";
        	} else {
      ...
      if (isset($news['from']) and $news['from'] == $_SESSION['id']) {
        echo "<div class=\"x\">";
			...
			echo "<!--div class=\"title\"><a href=\"/profile/news/get/{$news['id']}\" class=\"newsopen\" title=\"{$qwtitle}\">{$news['title']}</a></div-->";
			...
			if($news['medias']['found_rows']>count($news['medias']["firstline"])){
				echo "&nbsp;<span class=\"icons\"><a href=\"/profile/news/get/{$news['id']}#photos\">{$this->t('common','total photos')}&nbsp;({$news['medias']['found_rows']})</a></span>";
			}
