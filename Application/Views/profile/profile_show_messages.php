  ...
  foreach ($messages as $message) {
    ...
    */
    ...
    echo "<div class=\"message\" id=\"message{$message['id']}\">".
         ...
         "<div class=\"who\"><div class=\"thumb\" style=\"float:left; margin-right: 6px; width:50px; height:50px; background-image: url('{$message['thumbnail']}) ; background-repeat: no-repeat; background-position: center center;\"></div>{$message['name']}</a><br /><span>$created</span></div>".
        
         "<div class=\"preview\"$readStyle>$title<br />$preview</div>".
         "</div>".
    	 "<div id=\"dialog{$message['id']}\" title=\""
    	 .$this->t('profile','Delete message?')."\" style=\"display:none\">".
		 "<p><span id=\"dialogSpan{$message['id']}\"class=\"ui-icon ui-icon-alert\" style=\"float:left; margin:0 7px 20px 0;\">"
		 .$this->t('profile','Are you sure you want to delete this message?').		 
		 "</p></div>";
        
  }
} else {
  echo $this->t('profile',"No messages...");
}
